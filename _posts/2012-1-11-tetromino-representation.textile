...
Each Tetromino is composed of multiple equally sized sqaure segments laid out on a grid. A Tetromino ==\(t\)== can be represented as ==\(t\subset\mathbb{N}\times\mathbb{N}\)==, where each element of ==\(t\)== represents the center of one square in the tetronimo. For example, when the origin is at the lower left:
