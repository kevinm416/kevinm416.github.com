...
We will find solutions to instances of this problem using the A* search algorithm. The A* algorithm promises not only to find solutions to instances of these problems, but also to find optimal solutions. 
...
The key to easily implementing a solution to this problem is to interpret the problem in the form of a graph. Each node on the graph is a state of the puzzle, which will be represented by instances of the @PuzzleState@ class. Moves can be thought of as edges between states. 
...
Each @Car@ stores the position of its upper left hand corner in @(row, column)@. From there it extends @size@ squares in the direction specified by @direction@. Note that <code>Car</code>s are immutable; moving a car returns a new instance.
...
Finding a sequence of moves to take the puzzle from its initial state to a solution state is a 
...
@pathLength@ is the number of steps this state is from the initial state. We also store the heuristic used in A* so we do not need to recalculate it every time it is used.
...
The main use of @PuzzleState@ is to provide a list of its children: states that can be reached from the current @PuzzleState@ in one move. To assist with finding the children, we have defined the private static class @PuzzleBitmap@. 
...
We can determine if this is a solution state by checking if the rightmost column occupied by the special car is the edge of the board.
...
In the previous section, we quickly introduced @Puzzle@ as a place to put @aStar@. However, its reason for existence as a class is mainly to parse puzzle configurations.

A valid configuration file has an initial line with the board dimensions separated by whitespace. Each subsequent line has the form:
