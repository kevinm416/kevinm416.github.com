...
Let us write the code for this recurrence assuming we have the map @edit@ from @(Int, Int)@ to @Int@ representing the edit distances. 
...
All we needed to do was look up the three subproblems, and take the value of the subproblem that minimized the edit distance to the current position.

Combine that with the following base cases, and we have completely described the edit distance algorithm. These bases cases are straightforward; to turn the empty string into another string, we need to insert each character in the other string, or to turn a string into the empty string, we need to delete each character in that string.
...
We will add these base cases to the map @edit@, and then begin calculating intermediate edit distances so that we will be able to find ==\(edit[w1.length, w2.length]\)==.
...
If ==\(l = w1.length\)== and ==\(m = w2.length\)==, this algorithm requires filling in a table with ==\(lm\)== entries. Each of those indices in the table requires a call to @recurrence@. Haskell's array operations provide us with ==\(O(1)\)== time access to the characters in the input words. However, Haskell's maps have ==\(O(log(n))\)== insert and lookup times, so each call to @recurrence@ takes ==\(O(log(lm))\)== time. The overall runtime is therefore ==\(O(lm ~ log(lm))\)==. This runtime could be reduced by using a more efficient data structure for ==\(edit\)==, such as a mutable array. However, this approach was very straightforward.
