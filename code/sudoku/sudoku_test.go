...
func assertPuzzlesEqual(correct, attempt *puzzleState, t *testing.T) {
    if !correct.Equals(attempt) {
        t.Errorf("boards do not match correct, soln:\n%v\n%v", correct, attempt)
    }   
}
...
func TestDigitSet(t *testing.T) {
    ...
    for i := 1; i <= 9; i++ {
        ds = ds.Add(NewDigitSet(i))
        if ds.Size() != i {
            t.Errorf("ds.Size(): %v, but added: %v", ds.Size(), i)
        }
    }
}

func TestDigitSetOps(t *testing.T) {
    ds1 := EmptyDigitSet()
    ds1 = ds1.Add(NewDigitSet(1))
    ds1 = ds1.Add(NewDigitSet(3))
    ds1 = ds1.Add(NewDigitSet(5))

    ds2 := EmptyDigitSet()
    ds2 = ds2.Add(NewDigitSet(3))
    ds2 = ds2.Add(NewDigitSet(7))
    ds2 = ds2.Add(NewDigitSet(9))
    
    sub := ds1.Subtract(ds2)
    if uint16(sub) != 0x11 {
        t.Errorf("sub: %9b, shoud be 10001b", sub)
    }
}

func TestDigitSetToDigit(t *testing.T) {
    for i := 0; i < 9; i++ {
        if NewDigitSet(i).ToDigit() != i {
            t.Errorf("NewDigitSet(%v).ToDigit() -> %v", 
                i, NewDigitSet(i).ToDigit())
        }
    }   
}

func TestPuzzleState(t *testing.T) {
    correct := NewPuzzleState(
       `534678912
        672195348
        198342567
        859761423
    ...
    if ps.Solution() != true {
        t.Errorf("this should be a solution")
    }
...
func TestPuzzleState2(t *testing.T) {
    correct := NewPuzzleState(
       `483921657
        967345821
        251876493
